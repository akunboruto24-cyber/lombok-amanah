...
import { Clock, Users, MapPin, Star, Check, X as XIcon, ArrowLeft, Phone, Car, Calendar } from 'lucide-react';
import { getTourBySlug, getTourPackages, formatPrice, getSettings } from '@/lib/data';
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10">
          <div className="max-w-7xl mx-auto">
            <Link href="/tours" className="inline-flex items-center gap-2 text-white/60 text-[13px] hover:text-gold-400 transition-colors mb-4">
              <ArrowLeft className="w-4 h-4" /> Kembali ke Tour
            </Link>
            ...
            <div className="flex flex-wrap items-center gap-4">
              {tour.review_count && tour.review_count > 0 && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-white/40 text-[13px]">({tour.review_count} review)</span>
                </div>
              )}
              <span className="text-2xl font-bold text-gold-400">{formatPrice(tour.price)}<span className="text-[14px] text-white/40 font-normal">/grup</span></span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
                {tour.duration && (
                  <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl">
                    ...
                    <div>
                      <p className="text-[11px] text-navy-900/40 uppercase tracking-wide">Durasi</p>
                ...
                {tour.meeting_time && (
                  <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl">
                    ...
                    <div>
                      <p className="text-[11px] text-navy-900/40 uppercase tracking-wide">Mulai</p>
                ...
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-[14px] font-semibold text-navy-900">{tour.max_passenger} orang</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl">
                  <Car className="w-5 h-5 text-gold-400" />
                  <div>
                    <p className="text-[11px] text-navy-900/40 uppercase tracking-wide">Kendaraan</p>
            ...
            {tour.destinations && tour.destinations.length > 0 && (
              <section>
                <h2 className="text-xl font-display font-bold text-navy-900 mb-4">Destinasi</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {tour.includes.length > 0 && (
                <section className="p-5 bg-green-50 rounded-xl">
                  <h3 className="text-[14px] font-bold text-green-800 mb-3">Termasuk</h3>
              ...
              {tour.excludes.length > 0 && (
                <section className="p-5 bg-red-50 rounded-xl">
                  <h3 className="text-[14px] font-bold text-red-800 mb-3">Tidak Termasuk</h3>
                  <ul className="space-y-2">
            ...
            {tour.additional_info && (
              <section className="p-5 bg-blue-50 rounded-xl">
                <h3 className="text-[14px] font-bold text-blue-800 mb-2">Informasi Tambahan</h3>
            ...
            {tour.reviews && tour.reviews.length > 0 && (
              <section>
                <h2 className="text-xl font-display font-bold text-navy-900 mb-4">Review Pelanggan</h2>
